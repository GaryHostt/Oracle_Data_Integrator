...
- These two blog posts focus on connecting object storage and ADW/ATP to ODI. To connect to object storage in ODI, the PEM key you use must have a passphrase. In order to work with the compressed BICC extracts in object storage, you must first make an object storage topology and then the BICC toplology. If you have problems with loading your data into ADW, please consult the queries under STEP 7, in this [link](https://oracle.github.io/learning-library/workshops/journey4-adwc/?page=LabGuide3OOW.md). You may need to reconfigure the target tables in ADW to match the columns in your BICC extract if something like your column size is different from the default. 
